...
void request_table(int v, int &start)
{
	...
	Clist[(ver[v].Partition_id)].store(v);
...
//	pthread_mutex_lock(&lock[head_index.load()]);
	...
	while(true)
	{
		...
		{
			if(table[curr_row][i] == true)
			{
				int temp = Clist[i].load();	//which vertex of that partition?
				//check if this vertex is adjacent
	...
	table[curr_row][ver[v].Partition_id] = true;
...
//	temp.sort();
	...
	table[curr_row][ver[v].Partition_id] = false;
...
//	head.store(table[1]);
	...
	Clist = new atomic<int>[NTHREADS+1];
