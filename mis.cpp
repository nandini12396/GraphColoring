...
int numver, max_deg=0, p, NTHREADS;
...
void request_graph(int v, int pid)
{
	node* tp;
	node *ptr;
	...
	SG[pid].next = NULL;
	SG[pid].added.store(true);
//	SG[pid].active.store(false);
//	SG[pid].num_edges = 0;	

	tp = ver[v].next;
	while(tp != NULL)
	{
		int temp = ver[tp->id].Partition_id;
		if(temp == pid)		//neighbouring vertex from same partition
			goto l1;
		if(SG[temp].added.load() == false)
			goto l1;
		 	
		if(SG[pid].next == NULL)
		{
			SG[pid].next=(node*)(malloc(sizeof(node)));
			ptr = SG[pid].next;
			ptr->id = temp;
//			SG[pid].num_edges++;
			ptr->next = NULL;
...
//				SG[pid].num_edges++;				
		...
		if(SG[temp].next == NULL)
		{
			SG[temp].next=(node*)(malloc(sizeof(node)));
			ptr = SG[temp].next;
			ptr->id = pid;
//			SG[pid].num_edges++;
			ptr->next = NULL;
...
//	if(SG[pid].next == NULL)
//		SG[pid].active.store(true,std::memory_order_seq_cst);
	
	ptr = SG[pid].next;
	bool flag = false;

	if(ptr == NULL)
		SG[pid].active.store(true);
/*	else
	{
		if(SG[ptr->id].active.load() == true)
			flag = true;
...
void release_graph(int v, int pid)
{
	pthread_mutex_lock(&lock);

	SG[pid].active.store(false);

	SG[pid].added.store(false);
//	SG[pid].num_edges = 0;
	SG[pid].next = NULL;	
...
/*	for(i=0;i<NTHREADS;i++)
	{
		if(SG[i].added = true && SG[i].active == false && SG[i].num_edges == 0)
			flag = true;
	}
*/
...
//	node *ptr;
	...
	{
		//remove all neighbours of active vertices and active vertices themselves
		if(SG[*t].active.load() == true)
		{
			ptr = SG[*t].next;
			while(ptr!=NULL)
			{
				if(SG[ptr->id].added.load() == true)
				{
					//find that element in the list and then pass the iterator to remove it
					t2.push_back(ptr->id);
				}
				ptr = ptr->next;
			}	
	...
	for(list<int>::iterator t=t2.begin();t!=t2.end();t++)
	{
		temp = *t;
...
/*
	...
	{
		//remove all neighbours of active vertices and active vertices themselves
		if(SG[*t].active.load(std::memory_order_seq_cst) == true)
		{
			ptr = SG[*t].next;
			while(ptr!=NULL)
			{
				if(SG[ptr->id].added.load(std::memory_order_seq_cst) == true)
					t1.remove(ptr->id);					
				ptr = ptr->next;
			}	
			t1.remove(*t);
...
*/
	t2.clear();
	//here no active vertices are in the list
	while(t1.size() > 0)	
	{
		temp = t1.front();
		t2.push_back(temp);
...
int main(int argc,char *argv[]) 
{
	...
	SG = (subgraph*) malloc((NTHREADS+1) * sizeof(subgraph));

	for(i=0;i<NTHREADS;i++)
	{
		SG[i].pid = i;
		SG[i].added.store(false);
//		SG[i].num_edges = 0;
		SG[i].active.store(false);
		SG[i].next = NULL;		
	}
