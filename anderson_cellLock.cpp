...
void request_table(int v, tab** start)
{
	*start = head.load();
	list<int> temp;
	temp.clear();
	...
	for(list<int>::iterator t=temp.begin();t!=temp.end();t++)
		pthread_mutex_lock(&table[curr_row][*t].lock);
	...
	while(true)
	{
		...
		for(list<int>::iterator t=temp.begin();t!=temp.end();t++)
			pthread_mutex_lock(&table[next_row][*t].lock);
		
		for(list<int>::iterator t=temp.begin();t!=temp.end();t++)
			pthread_mutex_unlock(&table[curr_row][*t].lock);

		curr_row = next_row;
	}

	for(list<int>::iterator t=temp.begin();t!=temp.end();t++)
		table[curr_row][*t].val = 1;
...
void release_table(int v, tab** start)
{
	list<int> temp;
	temp.clear();
	node* tp;
...
int main(int argc,char *argv[]) 
{
	...
	table = new tab*[numver+1];
	for(int i = 1; i <numver+1; i++)
		table[i] = new tab[numver+1];

	for(i=1;i<=numver;i++)
		for(j=1;j<=numver;j++)
		{
			table[i][j].val = 0;
			pthread_mutex_init(&table[i][j].lock,NULL);			
		}
